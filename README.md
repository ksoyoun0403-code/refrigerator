...
- My Page에서 회원 탈퇴 시 계정·냉장고·세션 등 사용자 데이터 삭제
- 탈퇴 사용자가 공유한 레시피는 삭제하지 않고 작성자를 `익명`으로 전환
...
- 식재료 조회·수정, 여러 재료 선택 삭제 및 영역 간 일괄 이동·되돌리기
...
- 시·분 단위로 알림 시간 지정
- 앱 최초 진입 시 한 번만 알림 권한 안내를 표시하고 이후 알림 설정에서 다시 권한 확인 가능
- Android 12 이상에서 정확한 알람 권한 설정 화면 연결
- 현재 설정과 예약 키를 비교해 필요한 로컬 알림만 유지·갱신하여 중복 알림 방지
- 이미 지난 알림 시점은 임의 시간으로 옮기지 않고 제외
- 등록 완료 화면에서 실제 예약 시점과 제외된 시점 안내
- 알림 권한 비활성화 및 예약 확인 실패 상태 안내
- 단일 재료 알림을 누르면 해당 재료 상세로, 같은 시각의 여러 재료가 묶인 알림은 냉장고 탭으로 이동
...
- 냉장고 재료를 최대 12개까지 선택
- 선택한 냉장고 재료를 한 번에 모두 해제 가능
- 인원수 `1~10명`, 최대 조리 시간 `15~120분` 직접 입력 및 기본 양념 보유 여부 설정
- 기본 입력값은 `1명`, `30분`
- 보유 재료 중심 레시피와 추가 재료 1~3개가 필요한 레시피 생성
- 보유 재료만 사용하는 레시피 최대 2개와 추가 재료가 필요한 레시피 최대 2개 제공
- 사용자가 최근 공유한 레시피 제목 최대 30개를 참고해 동일·유사 제목 생성을 억제하고 동일 제목 결과를 후처리로 제외
...
- `인기순`과 `최신순` 정렬 지원
- 인기순은 북마크 수, 댓글 수, 최신 등록 시각 순으로 정렬
- 북마크 추가·해제 및 공개 북마크 횟수 표시
- 댓글 조회, 등록, 본인 댓글 수정·삭제
- `ADMIN_LOGIN_IDS`로 지정한 관리자는 다른 사용자의 댓글을 수정하지 않고 삭제만 가능
...
- 나의 요리책 우측 상단 `닫기`로 공유 레시피 목록 복귀
- 로그인 사용자 메뉴는 `My Page`에서 관리
- Android 최상위 화면에서 시스템 뒤로가기를 누르면 `MYDISH를 종료할까요?` 확인 후 종료
...
| 기기 기능 | Expo Image Picker, Notifications, Secure Store, Application, Intent Launcher |
...
- `back/.env.example`: Database, Vision, OpenAI, JWT, 관리자 계정 설정
...
ADMIN_LOGIN_IDS=관리자아이디
```

관리자가 여러 명이면 `ADMIN_LOGIN_IDS=admin1,admin2`처럼 로그인 ID를 쉼표로 구분합니다. 이 값은 Backend에서만 사용하며, 변경 후 Backend를 재시작해야 적용됩니다.
...
- `DELETE /v1/users/me` — 회원 탈퇴 및 공유 레시피 작성자 익명화
...
- `GET /v1/recipe-posts?q=&sort=popular|latest`
...
- 알림은 서버 푸시가 아닌 기기 로컬 예약 방식이며 댓글·멘션 원격 푸시는 미지원
- 대댓글과 `@닉네임` 멘션 연결·알림 미지원
