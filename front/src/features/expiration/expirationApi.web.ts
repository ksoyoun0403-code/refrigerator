...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new Error(`백엔드에 연결할 수 없습니다. API 주소(${API_BASE_URL})와 서버 상태를 확인해주세요.`);
  }
...
export function deleteExpirationItem(id: string) {
  return request<void>(`/expiration-items/${encodeURIComponent(id)}`, { method: 'DELETE' });
}

export function updateExpirationItem(id: string, input: UpdateExpirationItem) {
  return request<ExpirationItem>(`/expiration-items/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(input),
  });
...
export async function scanExpirationImage(image: LocalImage) {
  const imageResponse = await fetch(image.uri);
  const imageBlob = await imageResponse.blob();
  const form = new FormData();
  form.append('image', imageBlob, image.fileName);
